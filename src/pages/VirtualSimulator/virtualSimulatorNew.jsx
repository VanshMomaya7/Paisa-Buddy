...
import { TrendingUp, TrendingDown, Plus, Eye, BarChart3, PieChart, Wallet, Target, AlertCircle, Star, ArrowUpRight, ArrowDownRight, Wifi, WifiOff, Search, RefreshCw, Activity, TrendingUpIcon } from 'lucide-react';
...
const VirtualPortfolioSimulator = () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setMarketData(prevData => 
          prevData.map(item => {
            const apiData = result.data.find(stock => stock.symbol === item.symbol);
            if (apiData) {
              return {
                ...item,
                price: apiData.price,
  ...
  const handleBuy = () => {
    ...
    console.log(`✅ Bought ${buyQuantity} shares of ${selectedStock.name} for ₹${totalCost.toFixed(2)}`);
  };
  ...
  const handleSell = (holding, sellQuantity) => {
    ...
    console.log(`✅ Sold ${sellQuantity} shares of ${holding.name} for ₹${saleValue.toFixed(2)}`);
  };
  ...
  const ApiStatusIndicator = () => (
    <div className="flex items-center space-x-2 text-sm">
      ...
      <button
        ...
        className={`p-1 rounded hover:bg-gray-100 ${isRefreshing ? 'opacity-50' : ''}`}
      >
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className={`flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-medium ${
              marketSummary.marketTrend === 'BULLISH' ? 'bg-green-100 text-green-800' :
              marketSummary.marketTrend === 'BEARISH' ? 'bg-red-100 text-red-800' :
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">{marketSummary.summary.advancing}</div>
              <div className="text-sm text-gray-600">Advancing</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600">{marketSummary.summary.declining}</div>
              <div className="text-sm text-gray-600">Declining</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-gray-600">{marketSummary.summary.unchanged}</div>
              <div className="text-sm text-gray-600">Unchanged</div>
            </div>
            <div className="text-center">
              <div className={`text-2xl font-bold ${marketSummary.summary.averageChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <tbody className="divide-y divide-gray-200">
                {transactionHistory.slice(0, 10).map((transaction) => (
                  <tr key={transaction.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <div className="font-medium text-gray-900">{transaction.stock}</div>
                    ...
                    <td className="px-6 py-4 font-semibold">₹{transaction.total.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="mt-4 border-t pt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {searchResults.slice(0, 6).map((stock) => (
                <div key={stock.symbol} className="border rounded-lg p-3 hover:bg-gray-50">
                  ...
                  <div className="mt-2 flex space-x-2">
                    <button
                      onClick={() => {
                        const fullStock = marketData.find(s => s.symbol === stock.symbol) || {
                          ...stock,
                          id: stock.symbol,
                          name: stock.symbol,
                          type: 'stock',
            ...
            <tbody className="divide-y divide-gray-200">
              {marketData.map((instrument) => (
                <tr key={instrument.id} className="hover:bg-gray-50">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {instrument.source && (
                          <span className={`px-2 py-1 text-xs rounded-full ${
                            instrument.source === 'twelvedata' ? 'bg-green-100 text-green-800' :
                            instrument.source === 'fallback' ? 'bg-yellow-100 text-yellow-800' :
                            instrument.source === 'demo' ? 'bg-blue-100 text-blue-800' :
                            instrument.source === 'simulated' ? 'bg-purple-100 text-purple-800' :
                            'bg-gray-100 text-gray-800'
                          }`}>
                            {instrument.source === 'twelvedata' ? 'Live' : 
                             instrument.source === 'fallback' ? 'Cached' :
                             instrument.source === 'demo' ? 'Demo' :
                             instrument.source === 'simulated' ? 'Sim' : 'Loading'}
                          </span>
                        )}
                        {instrument.marketStatus && (
                          <span className={`px-2 py-1 text-xs rounded-full ${
                            instrument.marketStatus === 'OPEN' ? 'bg-green-100 text-green-800' :
                            instrument.marketStatus === 'CLOSED' ? 'bg-red-100 text-red-800' :
                            'bg-gray-100 text-gray-800'
                          }`}>
                            {instrument.marketStatus}
                          </span>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">
                    {instrument.volume ? instrument.volume.toLocaleString('en-IN') : 'N/A'}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-600">
              ...
              {watchlist.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-6 py-8 text-center text-gray-500">
                    No instruments in watchlist. Add some from the Markets tab or use the search feature!
                  </td>
                </tr>
              ) : (
                watchlist.map((instrument) => {
                  // Find current price from marketData
                  const currentData = marketData.find(item => item.id === instrument.id);
                    ...
                    <tr key={instrument.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{displayData.symbol}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 font-semibold">₹{displayData.price.toFixed(2)}</td>
                      <td className="px-6 py-4">
                        <div className={`flex items-center ${displayData.changePercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {displayData.changePercent >= 0 ? <ArrowUpRight className="w-4 h-4 mr-1" /> : <ArrowDownRight className="w-4 h-4 mr-1" />}
                          <div>
                            <div className="font-semibold">
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div className={`font-semibold ${selectedStock.changePercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div>
              ...
              {selectedStock.source && (
                <p className="text-xs text-gray-500">
                  Data Source: {selectedStock.source === 'twelvedata' ? 'Live Market Data' : 
                               selectedStock.source === 'fallback' ? 'Cached Market Data' : 
                               selectedStock.source === 'demo' ? 'Demo Data' :
                               selectedStock.source === 'simulated' ? 'Simulated Data' : 'Loading'}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Max affordable: {Math.floor(virtualCash / selectedStock.price)} shares
              </p>
            </div>
            ...
            <div className="bg-blue-50 p-3 rounded-lg">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold text-lg">₹{(selectedStock.price * buyQuantity).toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className="text-xs text-gray-500">Remaining Cash:</span>
                <span className="text-xs font-medium">₹{(virtualCash - (selectedStock.price * buyQuantity)).toFixed(2)}</span>
              </div>
            </div>
            ...
            <div className="flex space-x-3">
              <button
                onClick={handleBuy}
                disabled={selectedStock.price * buyQuantity > virtualCash || buyQuantity <= 0}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-gray-600">Learn investing with real Indian market data</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8">
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h4 className="font-semibold text-blue-900 mb-2">Investment Learning Tips & Features</h4>
              <ul className="text-blue-800 text-sm space-y-1">
                <li>• <strong>Real Data:</strong> Now using live/delayed market data from NSE/BSE via professional APIs</li>
                <li>• <strong>Search:</strong> Use the search bar in Markets tab to find specific stocks</li>
                <li>• <strong>Market Summary:</strong> Check overall market sentiment with top gainers/losers</li>
                <li>• <strong>Transaction History:</strong> Track all your buy/sell activities in the Dashboard</li>
                <li>• <strong>Diversify:</strong> Don't put all money in one stock or sector</li>
                <li>• <strong>Research:</strong> Check P/E ratios, market cap, and day high/low before investing</li>
                <li>• <strong>Long-term:</strong> Markets fluctuate short-term but tend to grow over the long term</li>
