...
import { Award, CreditCard, Calendar, CheckCircle, XCircle, Clock, TrendingUp, TrendingDown, AlertCircle, User, Phone, MapPin, Building2, Briefcase, DollarSign, BarChart3, Target, Shield } from 'lucide-react';
...
const CreditScoreSystem = () => {
  ...
  useEffect(() => {
    ...
    setPaymentHistory([
      { date: '2024-12-01', account: 'HDFC Credit Card', amount: 5000, status: 'On Time', impact: '+2' },
      { date: '2024-11-28', account: 'SBI Personal Loan', amount: 12500, status: 'On Time', impact: '+2' },
      { date: '2024-11-25', account: 'ICICI Credit Card', amount: 3500, status: 'On Time', impact: '+2' },
      { date: '2024-11-01', account: 'HDFC Credit Card', amount: 4800, status: '2 Days Late', impact: '-8' },
      { date: '2024-10-28', account: 'SBI Personal Loan', amount: 12500, status: 'On Time', impact: '+2' }
    ]);

    // Mock Credit Inquiries
    setInquiries([
      { date: '2024-11-15', type: 'Hard Inquiry', provider: 'Axis Bank', product: 'Home Loan', impact: '-3' },
      { date: '2024-09-22', type: 'Soft Inquiry', provider: 'Bajaj Finserv', product: 'Personal Loan', impact: '0' },
      { date: '2024-07-10', type: 'Hard Inquiry', provider: 'HDFC Bank', product: 'Credit Card', impact: '-5' }
    ]);
  }, []);
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg border">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-2xl font-bold ${getTotalCreditUtilization() <= 30 ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg border">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{inquiries.filter(inq => inq.type === 'Hard Inquiry').length}</p>
              ...
              <div key={key}>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-500">{factor.score}% of score</span>
                  </div>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className={`h-2 rounded-full ${
                      factor.current >= 80 ? 'bg-green-500' :
                      factor.current >= 60 ? 'bg-blue-500' :
                      factor.current >= 40 ? 'bg-yellow-500' :
                      'bg-red-500'
                    }`}
      ...
      <div className="bg-white rounded-xl shadow-lg border">
        <div className="p-6 border-b">
          ...
          <p className="text-gray-600">Simulate different scenarios to see their impact</p>
        </div>
        <div className="p-6">
          ...
          {creditHistory.length === 0 ? (
            ...
            creditHistory.map((activity) => (
              <div key={activity.id} className="p-6 flex items-center justify-between">
                ...
                <div className={`font-bold text-lg ${activity.impact >= 0 ? 'text-green-600' : 'text-red-600'}`}>
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                ₹{accounts.filter(acc => acc.type === 'Credit Card').reduce((sum, acc) => sum + acc.creditLimit, 0).toLocaleString('en-IN')}
              </p>
            </div>
            <div className="bg-blue-100 p-3 rounded-full">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">
                ₹{accounts.filter(acc => acc.type === 'Credit Card').reduce((sum, acc) => sum + (acc.creditLimit - acc.currentBalance), 0).toLocaleString('en-IN')}
              </p>
            </div>
            <div className="bg-green-100 p-3 rounded-full">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {accounts.map((account) => {
                const utilization = account.type === 'Credit Card' 
                ...
                return (
                  <tr key={account.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div>
                        <div className="font-semibold text-gray-900">{account.provider}</div>
                        <div className="text-sm text-gray-500">{account.type} • {account.accountNumber}</div>
                        <div className="text-xs text-gray-400">Opened: {account.openDate}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-semibold text-red-600">₹{account.currentBalance.toLocaleString('en-IN')}</div>
                      {account.minimumDue > 0 && (
                        <div className="text-sm text-gray-500">Min Due: ₹{account.minimumDue.toLocaleString('en-IN')}</div>
                      )}
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      {utilization !== null ? (
                        <div>
                          <div className={`font-semibold ${utilization <= 30 ? 'text-green-600' : utilization <= 70 ? 'text-yellow-600' : 'text-red-600'}`}>
                            {utilization}%
                          </div>
                          <div className="w-16 bg-gray-200 rounded-full h-2 mt-1">
                            <div 
                              className={`h-2 rounded-full ${utilization <= 30 ? 'bg-green-500' : utilization <= 70 ? 'bg-yellow-500' : 'bg-red-500'}`}
                    ...
                    <td className="px-6 py-4">
                      {account.currentBalance > 0 && (
                        <button
                          onClick={() => makePayment(account.id, Math.min(account.currentBalance, account.minimumDue || 5000), true)}
      ...
      <div className="bg-white rounded-xl shadow-lg border">
        <div className="p-6 border-b">
          ...
          <p className="text-gray-600">See how different payment scenarios affect your credit score</p>
        </div>
        <div className="p-6">
